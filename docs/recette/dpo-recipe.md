...
1. Business screens and routine API roles contain no code-to-contact export/search; only the double-validated DPO execution route can return encrypted ciphertext.
2. API and console execution require a named DPO, an organization-scoped request with independent DPO/legal validation, and 1–50 explicit public codes.
...
2. Expiry/draft cleanup preserves final submissions; complete retention deletes final data only after the approved response cutoff and is evidenced.
...
`DPO NO-GO` applies when sensitive processing lacks the required DPIA/approval, retention or rights remain undefined, small-cell/direct-contact leakage exists, identity access bypasses the double-validation procedure, or any critical evidence is missing.
