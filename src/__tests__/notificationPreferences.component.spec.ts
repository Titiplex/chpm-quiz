...
describe('NotificationPreferencesCard', () => {
  ...
  it('loads editable notification preferences and saves the selected policy', async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (String(url).endsWith('/notifications/subscriptions') && init?.method === 'POST') {
        return jsonResponse({
          subscription: { ...notificationSubscriptionFixture, frequency: 'immediate' },
        })
      }
    ...
    expect(wrapper.text()).toContain('Exécuter le digest quotidien')
    ...
    const saveCall = calls.find(
      (call) => call.method === 'POST' && call.url.endsWith('/notifications/subscriptions'),
    )
  ...
  it('shows the empty state and hides digest execution for a moderator', async () => {
    ...
    expect(wrapper.text()).not.toContain('Exécuter le digest quotidien')
  ...
  it('runs daily digests and displays the returned delivery count', async () => {
    ...
    await wrapper
      .findAll('button')
      .find((button) => button.text().includes('digest quotidien'))
      ?.trigger('click')
    await flushPromises()
